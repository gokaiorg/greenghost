...
## 2025-05-27 - Accessible Custom Dropdowns

**Learning:** Custom dropdown components built without accessible UI primitives (like Headless UI or Radix) must manually implement ARIA roles (`menu`, `menuitem`, `none`), `aria-expanded`/`aria-haspopup`, and an `Escape` key event listener to be usable by screen readers and keyboard users.
**Action:** When building or enhancing custom dropdowns, always bind an `Escape` key listener to close the menu and return focus to the trigger if appropriate, and use explicit `role` and `aria` attributes to define the interaction pattern.
