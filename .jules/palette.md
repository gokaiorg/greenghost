...
## 2024-05-18 - NavBurger Keyboard Accessibility
**Learning:** Custom dropdown components like the mobile hamburger menu (`NavBurger`) require full manual implementation of ARIA attributes (`role="menu"`, `role="none"`, `role="menuitem"`, `aria-expanded`, `aria-haspopup`, `aria-controls`), explicit focus indicators via Tailwind's `focus-visible`, and a global `keydown` event listener for the `Escape` key to meet accessibility standards and provide a robust keyboard navigation experience.
**Action:** When building custom interactive components that behave like standard UI primitives (e.g., dropdowns, modals, dialogs), always reference WAI-ARIA authoring practices to implement the correct roles, states, properties, and keyboard interactions from the beginning.
