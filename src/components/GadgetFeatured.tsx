...
export default async function GadgetFeatured() {
    ...
    const products = allProducts
        ...
        .sort((a, b) => featuredGadgetNames.indexOf(a.name) - featuredGadgetNames.indexOf(b.name))
        .slice(0, 6);
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <h2 className="text-xl lg:text-2xl font-bold text-left mb-2">Top 6 Must-Have <span className="text-[#13DE00]">Cannabis Accessory</span> in 2026</h2>
                        ...
                        <Link href={`/gadgets/${gadget.id}`} title={gadget.name}>
                            <div className={`hover:bg-[#13DE00]/13 p-1 flex flex-col relative cursor-pointer`}>
                                ...
                                <div className="flex justify-between flex-wrap">
                                    <p className="text-[10px] sm:text-xs text-gray-400 whitespace-nowrap">{gadget.status}</p>
                                </div>
