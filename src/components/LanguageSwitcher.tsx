  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      if (event.key === "Escape" && isOpen) {
        setIsOpen(false);
        // Optionally focus the toggle button after closing
            ...
            {languages.map((lang) => (
              <li key={lang.code} role="none">
                <button
                  ...
                  role="menuitem"
                  className={`w-full text-left px-4 py-2 text-xs font-medium flex items-center space-x-2 hover:bg-[#13DE00]/20 transition-colors cursor-pointer focus-visible:ring-2 focus-visible:ring-[#13DE00] focus-visible:outline-none
