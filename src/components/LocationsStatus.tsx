...
export default function LocationsStatus({ hours, slug }: LocationsStatusProps) {
  ...
  if (!mounted) {
    ...
    return (
      <div className="inline-flex items-center gap-2 px-3 py-1 bg-gray-800/90 rounded-sm min-w-[100px] h-[24px] animate-pulse"></div>
    );
  }

  return (
    <div
      className={`inline-flex items-center whitespace-nowrap gap-2 px-3 py-1 text-xs font-bold uppercase tracking-widest ${isOpen ? "bg-[#13DE00]/90 text-black" : "bg-red-500/90 text-white"}`}
    >
      <span
        className={`w-3 h-3 ${isOpen ? "bg-black" : "bg-white"} animate-pulse`}
      ></span>
