...
export default async function GardensList() {
    ...
    const sortedGroups = Object.entries(groupedGardens)
        .sort(([dateA], [dateB]) => {
    ...
    const jsonLd = {
        ...
        '@type': 'ItemList',
        'itemListElement': sortedGroups.flatMap(([date, items]) =>
            items.map((item) => ({
                '@type': 'ListItem',
                'item': {
                    '@type': 'Article',
        ...
        <div className="w-full max-w-4xl mx-auto py-12 px-4">
            ...
            <ul aria-label="Garden Updates Timeline" className="relative border-l-2 border-[#13DE00]/30 ml-3 md:ml-6 space-y-12">
                {sortedGroups.map(([date, items], groupIndex) => (
                    <li key={groupIndex} className="relative flex flex-col md:flex-row gap-6 md:gap-10 pl-8 md:pl-12">
                        {/* Timeline Dot */}
                        <div className="absolute -left-[9px] top-0 w-4 h-4 bg-[#13DE00] shadow-[0_0_10px_#13DE00] border-2 border-black" />

                        {/* Date Badge */}
                        <div className="md:w-32 flex-shrink-0">
                            <time dateTime={date.split('/').reverse().join('-')} className="inline-block px-3 py-1 bg-[#13DE00] text-black font-pixel font-bold text-sm shadow-[4px_4px_0px_#000000] border border-black">
                                {date}
                            </time>
                        </div>

                        {/* Group Content Cards */}
                        <ul aria-label={`Updates for ${date}`} className="flex-grow space-y-8">
