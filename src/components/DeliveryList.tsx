    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <section className="mb-12">
        ...
        <ol className="space-y-6" aria-label="How to order online steps">
          {steps.map((step, index) => (
            <li
              key={index}
              className="bg-[#13DE00]/5 border border-[#13DE00]/21 p-6"
            >
      ...
      {hints.length > 0 && (
        <section className="bg-[#13DE00]/5 border border-[#13DE00]/21 p-6 mb-12">
          <h2 className="text-2xl font-bold text-[#13DE00] mb-4">
      ...
      {steps.some((step) => step.video) && (
        <section>
          <h2 className="text-2xl font-bold text-[#13DE00] mb-6">
            Weed Delivery in Phuket Thailand
          </h2>
          {steps.map(
            (step, index) =>
              step.video && (
                <div
                  key={index}
                  className="aspect-video w-full overflow-hidden border border-[#13DE00]/21 bg-black"
                >
                  <iframe
                    width="100%"
                    height="100%"
