          ...
          {Array.from({ length: totalSlides }).map((_, slideIndex) => (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {products.slice(slideIndex * itemsToShow, (slideIndex + 1) * itemsToShow).map((product) => (
                  <Link key={product.id} href={`/strains/${product.id}`} title={product.name}>
                    <div className={`hover:bg-[#13DE00]/13 p-6 flex flex-col relative cursor-pointer`}>
                      <div className="relative mb-2">
                        ...
                        <button
                          ...
                          className="absolute bottom-1 left-1 !bg-[#13DE00] text-black px-3 py-2 text-xs hover:!bg-black hover:text-[#13DE00] cursor-pointer"
                        >                          Buy 1g
                        </button>
                      </div>
                      <h2 className="text-base md:text-lg font-semibold mb-2">{product.name}</h2>
                      <div className="flex justify-between flex-wrap">
                        <p className={`text-xs ${product.dominance && product.dominance.startsWith('Sativa') ? 'text-[#d1fee5]' :
                          product.dominance && product.dominance.startsWith('Hybrid') ? 'text-[#c0ef24]' :
                            product.dominance && product.dominance.startsWith('Indica') ? 'text-[#ee9cc9]' : 'text-gray-400'
                          }`}>{product.dominance}</p>
                        {product.thc && product.thc > 0 ?
                          <p className="text-xs text-gray-400">THC {product.thc}%</p> :
                          product.cbd && product.cbd > 0 ?
                            <p className="text-xs text-gray-400">CBD {product.cbd}%</p> : null
                        }
                      </div>
                      <p className="absolute top-0 right-2 bg-black text-[#13DE00] px-2 py-1 text-sm">{product.price}฿</p>
                    </div>
                  </Link>
                ))}
              </div>
