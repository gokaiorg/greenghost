  ...
  switch (action.type) {
    ...
    case "ADD_ITEM": {
      ...
      return {
        ...
        items: [
          ...state.items,
          { ...action.product, quantity: 1, menuType: action.menuType },
        ],
      };
    }
    case "REMOVE_ITEM":
      return {
        ...
        items: state.items.filter(
          (item) => item.id !== action.id || item.menuType !== action.menuType,
        ),
      };
    case "UPDATE_QUANTITY":
      if (action.quantity <= 0) {
        return {
          ...
          items: state.items.filter(
            (item) =>
              item.id !== action.id || item.menuType !== action.menuType,
          ),
        };
      }
      return {
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addItem = useCallback(
    (product: Product, menuType: string) =>
      dispatch({ type: "ADD_ITEM", product, menuType }),
    [],
  );
  const removeItem = useCallback(
    (id: string, menuType: string) =>
      dispatch({ type: "REMOVE_ITEM", id, menuType }),
    [],
  );
  const updateQuantity = useCallback(
    (id: string, quantity: number, menuType: string) =>
      dispatch({ type: "UPDATE_QUANTITY", id, quantity, menuType }),
    [],
  );
