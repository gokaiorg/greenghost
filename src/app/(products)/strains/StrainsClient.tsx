...
export default function StrainsClient() {
    ...
    // Fetch strains and page data on component mount
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch page data for SectionTitle
                const pageData = await getPageBySlug('strains')
                setPageData(pageData || null)
            } catch (error) {
    ...
    const filteredStrains = strains.filter(strain => {
        const matchesDominance = selectedDominances.length === 0 || selectedDominances.some(dominance =>
            strain.dominance.toLowerCase().includes(dominance.toLowerCase())
        )
        const matchesSearch = searchTerm === '' || strain.name.toLowerCase().includes(searchTerm.toLowerCase())
        return matchesDominance && matchesSearch
    })

    // Sort alphabetically by name
    const sortedStrains = filteredStrains.sort((a: Product, b: Product) => a.name.localeCompare(b.name))

    return (
        <>
            <Banner
                title="Strains"
                description="Explore our premium cannabis strains"
                bgSrc="/banners/green-ghost-best-degen-weed-shop-strains.avif"
                iconSrc="/images/icons/strains-green-ghost.avif"
                iconAlt="Green Ghost Strains"
            />
            ...
            {pageData?.section_title && (
                <h2 className="text-xl md:text-2xl font-bold text-center text-[#13DE00] container mx-auto px-4">
                    {pageData.section_title}
                </h2>
            )}
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                {loading ? (
                    ...
                    <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 list-none m-0 p-0" aria-label="Strains list">
