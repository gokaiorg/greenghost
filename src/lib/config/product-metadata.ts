...
export function generateProductSchema(product: Product) {
  ...
  const coverImage = product.images?.[0] ? `https://green.gd${product.images[0]}` : 'https://green.gd/images/logo-green-ghost-degen-weed-shop.png';
  ...
  return sanitizeSchema({
    ...
    name: product.name,
    description: product.description || `Browse ${product.name} - ${product.type} from Green Ghost weed shop in Bangkok, Thailand`,
    image: [coverImage],
    brand: {
      '@type': 'Brand',
      name: 'Green Ghost',
...
export function generateProductMetadata(product: Product): Metadata {
  ...
  const description = product.seo || product.description || `Browse ${product.name} - ${product.type} from Green Ghost weed shop in Bangkok, Thailand`;
